...
"-----------------------------------------------------------------------------
let javadoc_path="/Users/thomas/Documents/JavaApi/api"
" Windows users....
...
"-----------------------------------------------------------------------------
nmap <C-s> :call OpenJavadoc()<CR><CR>
"-----------------------------------------------------------------------------
"
		...
		if ((match(lfile, "\*\.html$") != -1) && has("gui_running"))
			...
			if (isdirectory(expand(lfile)))
				if has("win32")
					...
					let null = system(g:browser.' '."file://".lfile)
				endif
...
function! s:java_apidoc_searchHtml(path, classname)
	...
	while (lineno > 0)
		...
		if has("win32")
			...
			let null = system(g:browser.' '."file://".a:path.s:slash.fpath)
		endif
		let s:found = s:found + 1
		let lineno = search('<a href="[a-z/]*/'.a:classname.'\.html\c', 'W')
	endwhile
